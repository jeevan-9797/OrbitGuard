...
export const EarthOrbitalTwin3D: React.FC<EarthOrbitalTwin3DProps> = ({
  ...
  const [animSpeed, setAnimSpeed] = useState<number>(1); // calibrated 1x orbital velocity multiplier based on Earth gravity and satellite mass
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      if (isPlaying) {
        // Physical Keplerian angular velocity derived from Earth mass (5.972e24 kg), satellite mass (120 kg),
        // and orbital altitude (541.8 km) with gravitational parameter mu = G*(M_earth + m_sat)
      ...
      // Earth sidereal rotation in exact physical proportion to satellite orbital motion (1 / 15.04)
      const gmst = getGMST(currentDate) + (isPlaying ? animTimeRef.current * EARTH_TO_ORBIT_ROTATION_RATIO : 0);
      const sunVec = solarBetaData.sunVectorECI; // [x, y, z] in ECI
      ...
      const project = (x: number, y: number, z: number): { sx: number; sy: number; depth: number } => {
        ...
        return {
          sx: cx + x2 * persp,
          sy: cy - z2 * persp,
          depth: y2, // larger depth is closer/in-front
        };
      };
      ...
      if (showOrbitRings) {
        ...
        for (let i = 0; i < numSegments; i++) {
          ...
          // If both points are behind Earth's globe
      ...
      // -------------------------------------------------------------
      // 3. Draw Earth Wireframe Sphere (Solid Base + Grid + Continents)
      // -------------------------------------------------------------
      // Base dark globe
      ctx.save();
      ...
      // Clip subsequent continent & grid rendering inside the Earth disk
      ctx.save();
      ctx.beginPath();
      ctx.arc(cx, cy, earthRadius, 0, Math.PI * 2);
      ctx.clip();

      // 3a. Latitude Rings (Parallels)
      ctx.save();
      ...
      // 3c. Continental Coastlines (Rotating in real-time with GMST)
      if (showContinents) {
        ctx.save();
        ctx.strokeStyle = 'rgba(74, 222, 128, 0.65)'; // Glowing emerald coastlines
        ctx.lineWidth = 1.3;
      ...
      if (showTerminator) {
        ctx.save();
        // The Sun direction in view space dictates which hemisphere is illuminated
      ...
      // Sub-satellite Ground Footprint (Nadir Ground Spot)
      ...
      ctx.restore(); // Restore clipping region of Earth disk

      // -------------------------------------------------------------
      // 4. Draw Front-Half of Orbital Ring (In-Front of Earth, depth >= 0)
      // -------------------------------------------------------------
      if (showOrbitRings) {
        ctx.save();
        const numSegments = 90;
      ...
      if (showSunVector) {
        ...
        // Sun vector line from Earth center outwards
        ctx.beginPath();
        ctx.moveTo(cx, cy);
        ctx.lineTo(sunTip.sx, sunTip.sy);
        ctx.strokeStyle = '#f59e0b'; // Amber sun vector
        ctx.lineWidth = 1.8;
        ctx.stroke();
        ...
        ctx.fillText(`SOLAR VECTOR ☉ [β = ${solarBetaData.betaDeg > 0 ? '+' : ''}${solarBetaData.betaDeg.toFixed(1)}°]`, sunTip.sx + 10, sunTip.sy - 4);
        ctx.restore();
      }
      ...
      // -------------------------------------------------------------
      // 7. Draw the Animated 3D Digital Twin Satellite Wireframe Model!
      // -------------------------------------------------------------
      ctx.save();
      ...
      ctx.strokeStyle = '#22c55e'; // Green velocity vector
      ctx.lineWidth = 1.5;
      ctx.stroke();

      // Satellite body size in pixels
      const scale = Math.max(12, 18 * zoom);

      // Draw 3D Satellite Chassis Wireframe (Cube / Hexagonal prism)
      const primaryColor = inShadow ? '#fb7185' : '#38bdf8'; // rose if eclipse, cyan if sunlit
      const accentColor = inShadow ? '#f43f5e' : '#4edea3';
      ...
      // Inner Bus Diagonal Cross-Bracing (Wireframe Truss)
      ctx.beginPath();
      ...
      // Parabolic Dish Antenna (Pointing toward Earth)
      ctx.beginPath();
  ...
  return (
    <div className="relative w-full h-[460px] sm:h-[500px] bg-[#05070a] overflow-hidden rounded-2xl flex items-center justify-center select-none">
      ...
      {/* Top Left: Perspective / Camera Mode Controls */}
      ...
      <div className="absolute top-3 right-3 flex items-center gap-2 z-10">
        <div className="bg-[#0f172a]/90 border border-[#1e293b] px-3 py-1.5 rounded-xl flex items-center gap-2 backdrop-blur-md text-xs font-mono">
          ...
          <span className="text-amber-400 font-bold">β {solarBetaData.betaDeg > 0 ? '+' : ''}{solarBetaData.betaDeg.toFixed(1)}°</span>
      ...
      <div className="absolute bottom-3 right-3 flex items-center gap-1.5 bg-[#0f172a]/90 p-1.5 rounded-xl border border-[#1e293b] backdrop-blur-md z-10">
        ...
        {/* Speed Selector (Calibrated Keplerian Scale) */}
        <div className="flex items-center gap-1 px-1">
          {([0.5, 1, 2, 4] as const).map((spd) => (
            <button
              ...
              title={spd === 1 ? '1x Physically Calibrated Speed' : `${spd}x Simulation Multiplier`}
            >
              {spd === 1 ? '1x (CAL)' : `${spd}x`}
            </button>
          ))}
...
// Simple globe icon placeholder for button
